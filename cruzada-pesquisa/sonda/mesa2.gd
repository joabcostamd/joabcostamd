...
# --- JANELA DA COLHEITA ---
var madura := PackedInt32Array()   # 1 = linha cheia esperando na grade
var jan := PackedInt32Array()      # posicionamentos restantes de espera
var maduras_criadas := 0
var ult_mordeu := false            # o teto 24+4*rodada morderia neste evento?
var vazias_min := 25
...
static func cfg_padrao() -> Dictionary:
	return {
		...
		# --- NUCLEO POLIDO (bancada 3, proposta 5) ---
		"produto": false,      # mults multiplicam entre si e o Tear e FATOR
		"tear_ini": 0,         # 1 no modo produto (senao o fator zera)
		"teto_evento": true,   # false remove o teto 24+4*rodada
		"meta_k": 1.0,         # constante da curva de metas
		# --- CANDIDATO 4: JANELA DA COLHEITA ---
		"janela": 0,           # 0 = R17 imediata (base). 1, 2 = turnos. 99 = ate o fim da mesa
		"janela_junta": 0,     # 0 = qualquer par de linhas cheias; 1 = so linhas que compartilham carta
		"janela_max_por_mesa": 0,   # antidoto 1: 0 = sem limite; 1 = so a primeira linha madura da mesa
		"janela_cond_perp": false,  # antidoto 2: so amadurece se alguma perpendicular ja estiver em 4/5
		"cruz_mult_soma": false,    # true = mults SOMAM entre si (x Tear); false = mults MULTIPLICAM (b3)
	}
...
func _init(p_rodada: int, p_tipo: int, semente: int, p_cfg: Dictionary) -> void:
	...
	rodada = p_rodada
	tipo = p_tipo
	meta = int(round(float(METAS[p_tipo][p_rodada - 1]) * float(p_cfg.get("meta_k", 1.0))))
	...
	madura.resize(12); madura.fill(0)
	jan.resize(12); jan.fill(0)
	tear = int(p_cfg.get("tear_ini", 0))
	if semente == -1:
		return
...
func fator_linha(cat: int) -> float:
	if cfg["produto"]:
		return float(max(1, Nucleo.CAT_MULT[cat])) * float(tear)
	return float(Nucleo.CAT_MULT[cat] + tear)
...
func _pontos_evento(fechadas: Array, casa_ov: int, carta_ov: int, fix: Dictionary) -> Array:
	...
	for li in fechadas:
		...
		produto *= float(max(1, mult_de(cat)))
		parcelas.append([fichas, Nucleo.DIAG[li]])
	var total := 0
	if cfg["produto"]:
		var fator: float = (float(soma_mult) if cfg["cruz_mult_soma"] else produto) * float(tear)
		ult_mordeu = fator > float(teto)
		if cfg["teto_evento"]: fator = min(float(teto), fator)
		for p in parcelas:
			var bruto: float = float(p[0]) * fator
			if p[1]: bruto *= 0.6
			total += int(floor(bruto))
	else:
		var soma: int = soma_mult + tear
		ult_mordeu = soma > teto
		var mult_ef: int = (min(teto, soma) if cfg["teto_evento"] else soma)
		for p in parcelas:
...
# ------------------------- JANELA DA COLHEITA ------------------------------
func linhas_maduras() -> Array:
	var r := []
	for li in range(12):
		if madura[li] == 1: r.append(li)
	return r

static func _cruzam(a: int, b: int) -> bool:
	for c in Nucleo.LINHAS[a]:
		if Nucleo.LINHAS[b].has(c): return true
	return false

# conjunto de linhas colhidas neste instante.
# novas = linhas que ficam cheias com o posicionamento; expirando = maduras com jan<=1
func _conta_pos(li: int, casa: int) -> int:
	var n: int = conta[li]
	if casa >= 0 and Nucleo.LINHAS[li].has(casa): n += 1
	return n

# a linha li pode ficar MADURA (esperar) ou tem de colher na hora?
func _pode_esperar(li: int, casa: int) -> bool:
	var mx := int(cfg["janela_max_por_mesa"])
	if mx > 0 and maduras_criadas >= mx: return false
	if cfg["janela_cond_perp"]:
		for lj in range(12):
			if lj == li: continue
			if _conta_pos(lj, casa) == 4 and _cruzam(li, lj): return true
		return false
	return true

func _alvo(novas: Array, ultimo: bool, expirando: Array, casa: int) -> Array:
	var cheias := linhas_maduras()
	for li in novas:
		if not cheias.has(li): cheias.append(li)
	if cheias.is_empty(): return []
	if ultimo: return cheias
	var gat := expirando.duplicate()
	for li in novas:
		if not _pode_esperar(li, casa): gat.append(li)
	if gat.is_empty(): return []
	if int(cfg["janela_junta"]) == 0: return cheias
	var sel := {}
	for li in gat: sel[li] = true
	var mudou := true
	while mudou:
		mudou = false
		for li in cheias:
			if sel.has(li): continue
			for lj in sel.keys():
				if _cruzam(li, int(lj)):
					sel[li] = true; mudou = true; break
			if mudou: break
	return sel.keys()

func _expirando() -> Array:
	var e := []
	for li in range(12):
		if madura[li] == 1 and jan[li] <= 1: e.append(li)
	return e

# um posicionamento QUALQUER ja dispara colheita?
func gatilho_pendente() -> bool:
	if int(cfg["janela"]) <= 0: return false
	if posic_usados + 1 >= posic_max: return true
	return not _expirando().is_empty()
...
func ganho(casa: int, carta: int) -> Array:
	...
	var alvo := _alvo(fecha, posic_usados + 1 >= posic_max, _expirando(), casa)
	if alvo.is_empty(): return [0, 0]
	var r2 := _resolver_evento(alvo, casa, carta)
	return [r2["pontos"], alvo.size()]
...
func valor_pulso(li: int, casa_nova: int, carta_nova: int, F: float) -> int:
	...
	var v := float(base_de(cat) + fic) * fator_linha(cat) * F
...
func posicionar(carta: int, casa: int, turno: int) -> Array:
	...
	for li in Nucleo.CELL_LINHAS[casa]:
		if conta[li] == 4: fecha.append(li)
	var J := int(cfg["janela"])
	var alvo := fecha
	var ultimo := (posic_usados + 1 >= posic_max)
	if J > 0:
		alvo = _alvo(fecha, ultimo, _expirando(), casa)
	...
	if not alvo.is_empty():
		var r := _resolver_evento(alvo, casa, carta)
		mordeu = ult_mordeu
		pev = r["pontos"]; nlin = alvo.size(); com_cor = r["com_coringa"]; ids = r["ids"]
	...
	if J > 0:
		for li in range(12):
			if madura[li] == 1: jan[li] -= 1
		for li in fecha:
			if madura[li] == 0 and not alvo.has(li):
				madura[li] = 1
				jan[li] = (999999 if J >= 99 else J)
				maduras_criadas += 1
	if nlin > 0:
		pontos += pev
		if mordeu: stats["teto_mordeu"] += 1
		if nlin >= 2:
			stats["cruz_n"] += 1
			if mordeu: stats["cruz_teto_mordeu"] += 1
			var perp := false
			for i in range(alvo.size()):
				for j in range(i + 1, alvo.size()):
					if _cruzam(int(alvo[i]), int(alvo[j])): perp = true
			if perp: stats["cruz_perp"] += 1
			stats["cruz_nlinhas"].append(nlin)
		stats["n_eventos"] += 1
		stats["pontos_total_eventos"] += pev
		if com_cor: stats["pontos_com_coringa"] += pev
		stats["colheitas"] += 1
		if com_cor: stats["colheitas_com_coringa"] += 1
		_colher(alvo, ids, turno)
		if J > 0:
			for li in alvo:
				madura[int(li)] = 0
				jan[int(li)] = 0
			for li in range(12):
				if madura[li] == 1 and conta[li] < 5:
					madura[li] = 0
					jan[li] = 0
	...
	var nv := 0
	for c in range(25):
		if grade[c] == VAZIA: nv += 1
	if nv < vazias_min: vazias_min = nv
	return [pev, nlin, pp]

# colhe o que sobrou maduro quando a mesa acaba (regra 5 do candidato 4)
func finalizar_janela(turno: int) -> Array:
	if int(cfg["janela"]) <= 0: return [0, 0]
	var alvo := linhas_maduras()
	if alvo.is_empty(): return [0, 0]
	var r := _resolver_evento(alvo, -1, -1)
	var pev: int = int(r["pontos"])
	var mordeu := ult_mordeu
	pontos += pev
	stats["n_eventos"] += 1
	stats["pontos_total_eventos"] += pev
	stats["colheitas"] += 1
	...
	if alvo.size() >= 2:
		...
		for i in range(alvo.size()):
			for j in range(i + 1, alvo.size()):
				if _cruzam(int(alvo[i]), int(alvo[j])): perp = true
		if perp: stats["cruz_perp"] += 1
		stats["cruz_nlinhas"].append(alvo.size())
	_colher(alvo, r["ids"], turno)
	for li in alvo:
		madura[int(li)] = 0
		jan[int(li)] = 0
	tear = min(int(cfg["tear_teto"]), tear + alvo.size())
	return [pev, alvo.size()]
...
func colheita_final() -> int:
	...
	for li in range(12):
		...
		var cat: int = r[0]
		var fichas: int = base_de(cat) + int(r[1])
		var v := float(fichas) * fator_linha(cat) * 0.5
		if Nucleo.DIAG[li]: v *= 0.6
		total += int(floor(v))
	pontos += total
	return total
