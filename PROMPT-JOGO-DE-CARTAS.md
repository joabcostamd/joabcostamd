...
5. No fim você recebe a pasta `placard/`, três documentos, capturas em `capturas/` e commit na branch `feat/cruz` (push, se houver remoto).
...
| F2 Núcleo | 4 (cartas/mãos, grade/mesa, selos/pipeline, aleatório/serialização) | `nucleo_testes.gd` com **≥ 120 asserções de núcleo** passando (avaliador, grade, colheita, cruz, Tear, determinismo, fita), 0 travamentos na varredura rápida |
...
**Fim do trabalho:** `git add`, `git commit` e, havendo remoto, `git push` em `feat/cruz`, criada a partir da branch atual; **nunca** commit direto na branch padrão. `push` que falha por ausência de remoto ou de credencial vira `PUSH — falhou — <motivo>` em `AUDITORIA.md`, o commit local permanece e o item do DoD conta como cumprido.
...
- **R12 PONTOS DE UMA LINHA** — `(fichas base da mão + soma das fichas das 5 cartas) × min(teto_do_evento, mult da mão + Tear vigente)`, e o resultado é multiplicado por **0,60 (piso)** se a linha for diagonal sem Bússola/Trama. O `teto_do_evento` é o da §5.1 e vale igualmente para linha simples e para cruz — não existe segundo teto.
- **R13 TABELA** — seção 5.
...
- **R17 CUSTO ESPACIAL** — colher esvazia 5 casas e reduz em 1 carta cada linha perpendicular. Pontuar sempre desmancha algo; a cruz é a única colheita sem sobra pela metade.
...
- **R39 O TEAR** — inteiro da mesa: começa em 0, **+1 por linha colhida** (diagonal inclusive), teto 8, **zera entre mesas**. Entra **uma vez por evento de colheita**, somado ao mult total, inclusive na cruz. O Tear **vigente antes do evento** é o que conta; só depois ele sobe. Nunca desce. É o único número que sobe a mesa inteira: sempre na tela, contando alto, subindo um semitom por degrau.
...
5. **Compra.** Escada de sinal: out comum = **silêncio**; out que leva uma linha a 4/5 = pulso e `ping_out`; out que fecha cruz = pulso triplo, as duas linhas acendem e o som ganha a quinta.
...
**Turno 14 — mão K♣, 4♦, 2♠, J♦, 5♥.** A tentação: 5♥ em C3 fecharia LINHA 3 = Trinca, fichas `30 + (10+10+5+10+9) = 74`, mult 3; e COLUNA C = Flush, fichas `35 + (10+7+5+6+8) = 71`, mult 4. Cruz com Tear 1: `(74+71) × (3+4+1) = 145 × 8 = 1.160`, levando a 2.340/2.962. Mas sobrariam **5 posicionamentos**, o tabuleiro ficaria **vazio** e faltariam **622 pontos** de uma linha do zero com 5 cartas forçadas — com Tear 2, Carta Alta paga ~150 e Par ~200; só trinca, flush ou cruz de sorte pagam 622. **Ele recusa** e guarda o 5♥ como plano B. **Ação:** descarte 2/3 (saem K♣, 4♦, 2♠; entram 8♣, 3♣, A♥ — sem 9); posiciona **J♦ em B5** (linha 5 nunca fecha aqui: lixeira grátis que alimenta a coluna B); compra 2♥.
...
- **CRUZ** = `(88 + 140) × min(40, 4 + 8 + Tear 1) = 228 × 13 = 2.964`. O teto da rodada 4 é `24 + 4×4 = 40`; 13 passa longe, o teto **não** morde.
...
**As asserções obrigatórias do teste, e só elas:** um evento `linha_fechou` FULL com `fichas_base=40`, `fichas_cartas=48`, `mult=4`; um `linha_fechou` SEQ_COR com `fichas_base=100`, `fichas_cartas=40`, `mult=8`; um `cruz` com `fichas=228`, `mult=13`, `pontos=2964`; `pontos` total `4144`; `meta_batida` presente; **nenhum** `colheita_final` com linha de 3+; e o índice de todo `removeu`/`reproposta` **maior** que o do `pontos`.
...
**Fórmula de um evento:** `pontos = (Σ fichas base + Σ fichas das cartas) × min(teto_do_evento, Σ mults + Tear vigente)`, com `teto_do_evento = 24 + 4 × rodada` (R15). É a **mesma** fórmula para linha simples e para cruz: a linha simples é a cruz de uma mão só. Não existe outro teto no jogo.
...
|---|---|---|---|---|
| Pequena | 15 × 4,2 = 63 s | 3 colheitas × 1,34 + 1,5 cruzes × 2,62 + cauda 1,5 = **9,4 s** | **72 s** | **66–84 s** |
...
- **Cruz com Tear 4:** `(130 + 210) × min(48; 7 + 10 + 4) = 340 × 21 = 7.140 ≥ 5.975` ✔ (o teto da rodada 6 é 48; não morde).
...
| 6 | Cruz só soma mults se as mãos forem de categorias diferentes | geométrico |
...
2. Mediana de cruzes por mesa **1,5–2,5**.
3. Razão cruz ÷ duas linhas simples **1,6×–2,2×**.
...
| `MULT_ADD_SE_CRUZ` | avaliação | `{n}` | Nó de Ouro |
...
| Nó de Ouro | E | 9 | Cruz que passa aqui soma +4 no mult | numérico |
...
| **Sino de Ouro** | Sino + Nó de Ouro (mesma casa) | toda cruz que passa ali paga +4 mult e $1 | sim |
...
| **Nós** | Nó Cego (+3 no Tear agora) · Cruz de Sal (+4 no mult da próxima cruz) · Contra-Nó (cancela o modificador desta mesa) · **Marcar** (declara a categoria de uma linha; cumprindo aquela categoria ou acima, a colheita paga +50%; falhando, **nada acontece** — sem punição) |
...
| Da Bússola | Diagonais pagam 100%; 2 casas lacradas | **6 cruzes numa run** |
| Da Viúva | Mão de 6; −2 posicionamentos por mesa | vencer uma run |

Cada baralho guarda **recorde próprio** (melhor run, melhor mesa, maior cruz) e placar próprio no Infinito.
...
| 6 | **O Tear Preso** | O Tear só sobe em cruz; colheita simples não o move | "Só cruz levanta o tear." | "Monte duas linhas antes de fechar uma." |
...
Tear de Chumbo (teto do Tear vira 16 / só sobe em cruz) · Casa Queimada (+6 mult global / 1 casa lacra por rodada) · Mão Larga (mão de 7 / −2 posicionamentos) · Naipe Único (Flush paga ×2 / Sequência de Cor não existe) · Faca de Dois Gumes (cruz +30% / linha simples −20%).
...
**Matriz de Cruzes:** grade simétrica **11×11** (categoria da linha × da coluna), carimbo espelhado nas duas células com coordenada, carta e pontos ("9♦ em C3 — 2.964"). São 66 pares únicos; os 11 com Quina entram quando a rota é desbloqueada. Célula vazia pode conter o **carimbo fantasma** da §7.2.
...
**Três estados de ficha:** *Silhueta* (forma escurecida + família + raridade + pista de 3 palavras), *Vista* (cinza), *Possuída* (texto completo, borda e uso: "colado 14× · 3 cruzes"). Nenhuma silhueta é 100% misteriosa.
...
| Fio de Ouro | `maior_cruz_pontos >= 5000` |
...
**34 estatísticas rastreadas** em `Progresso.estatisticas`, e os 12 predicados acima só podem usar campos desta lista: runs iniciadas/vencidas, vitória % por baralho e por Tabuleiro, tempo jogado, mesas vencidas, pontuação máxima de mesa e de run, maior cruz (coordenada + carta), Tear máximo, colheitas por categoria, **mão favorita** (mais colhida), **mão traidora** (mais perdida em 4/5), mediana de cruzes, 25 contadores por casa, casas com selo distintas, máximo de selos numa run, compras na loja, itens descobertos, dinheiro, descartes, Fianças, Novelos, motores disparados, turno mediano, sequência do Diário.
...
**Infinito** (desbloqueado na 1ª vitória): rodadas 7+ com `meta(r) = 5.975 × 1,42^(r-6) × (1 + 0,06 × (r-6))` — superlinearidade leve que mata a run entre as rodadas **12 e 18**. O `teto_do_evento` continua `24 + 4 × min(rodada, 12)`. O **Tear decai 50% entre mesas em vez de zerar**. Cada rodada sorteia +1 modificador (máx. 2 por mesa) e, a cada 3 rodadas, 1 de 3 Fardos permanentes. Perder congela e registra a **Marca d'Água** ("rodada 13 · 4,1M"); acima de 1e6, notação compacta. **Zen:** sem meta nem derrota, um recorde só — a maior cruz da sua vida, exibida no menu.
...
| Cruz | 1–2 por mesa | mediana 1,5–2,5 |
| Carimbo novo na Matriz | 2,8 por run | ≥ 1 em 90% das runs |
| Conquista | 1 a cada 1,6 run (runs 1–20) | ≥ 30 nas 40 primeiras runs |

**Marcos celebrados** (moldura dourada + acorde, 1,2 s, **nunca modal**): 1ª cruz, 1ª quadra, Tear 5, 10.000 pontos numa mesa, recorde batido, **motor novo disparado**, 100% de uma aba.
...
| Prévia fantasma | 980,64,300,532 | Painéis **LINHA**, **COLUNA** e, quando houver, **DIAGONAL**, com 5 cartas em miniatura, categoria e `fichas × mult = pontos`, tudo lido da fita de `Mesa.simular()`. Fechando, o cabeçalho vira `COLHE` e um bloco lista o que sai das perpendiculares. Sem carta na mão, mostra a **Galeria de Cruzes** da run; sem jogada boa, a **descarga nomeada** |
...
Texto de 14 px em dourado sob a barra de meta: `faltam 1.892 — uma cruz de Flush, ou duas trincas`. **Algoritmo, com custo limitado:** `Receita.para_deficit(deficit, tear, nivel_de_mao, rodada)` percorre a tabela da §5.1 **de cima para baixo** e devolve **no máximo duas** receitas — a menor cruz de duas categorias que fecha o déficit e a menor repetição de uma categoria só —, montadas com fichas médias de 45 por mão. Custo: ≤ 121 avaliações aritméticas, **recalculado só quando `pontos` ou `tear` mudam**, nunca por quadro. Chaves i18n: `partida_minimo_cruz` (`faltam %d — uma cruz de %s`), `partida_minimo_repeticao` (`ou %d %s`), `partida_minimo_fora` (usa a frase da R41). **Sem receita possível, a R41 assume** — o campo nunca fica vazio nem mente.
...
| 6 | Cruz: câmera lenta, `×2 + ×4 = ×6` | "CRUZ." | 6 s |
| 7 | Meta batida, garantida por seed | "Sua mesa. Próxima é 5×5." | 4 s |

Nos 10 primeiros segundos o jogador já viu **uma colheita destruindo uma perpendicular** e caminha para a **cruz**. Pulável a qualquer momento, refazível em Opções → Jogo.
...
**CRUZ — 2.620 ms:** igual até 240 ms; as duas linhas acendem em cruz (240–420); **hitstop 90 ms** em 420; `time_scale` cai a **0,35 por 600 ms** e volta em 180 ms; as 9 cartas pontuam em **duas escadas paralelas** (mão A nos graus 0–4, mão B nos 5–9) e a **carta do cruzamento toca as duas notas mais a quinta** — a assinatura sonora do jogo; em 1.500 os mults colidem estampando `×4 + ×8 = ×12` (2,0→1,0, 260 ms, BACK OUT); em 1.700 explosão em cruz, zoom-punch 1,06 e chuva de fichas; fronteira em 2.100. Cruz com diagonal acrescenta uma terceira escada nos graus 10–14 e a estampa vira `×4 + ×8 + ×4 = ×16`.
...
**Turbo:** `Efeitos.fator = velocidade_animacao (0,5–3,0) × (turbo ? 2,5 : 1,0)`; todas as durações e agendamentos são divididos pelo fator. Turbo **não** toca em regra, RNG, pontuação nem música. **Pisos inegociáveis:** clímax da cruz ≥ 380 ms; contagem de pontos ≥ 120 ms; a escada toca **todas** as notas, sobrepostas se preciso; hitstop cai a 40%, nunca a 0; a cauda de recompensa cai a 600 ms, nunca some. Toque durante animação = turbo momentâneo daquele beat; duplo toque chama `pular_tudo()`. As métricas de turno registram o fator usado, para a mediana não ser mascarada por aceleração visual.
...
| `cruz` | arpejo 8 notas + coral | A3→A5 | 5/60/200/400 | 1,25 s | −4 dB |
...
**Escada ascendente (obrigatória):** lá menor pentatônica, semitons a partir de A3 (220 Hz): `[0,3,5,7,10,12,15,17,19,22]`. **Um único** `nota_ficha.wav` com `pitch_scale = 2^(semitons/12)` — afinação exata, repositório pequeno. Linha simples: cartas 1→5 nos graus 0–4. Cruz: mão A nos graus 0–4, mão B nos 5–9, carta do cruzamento tocando as duas notas mais a quinta. **Tear:** `pitch_scale = 2^(min(tear,12)/12)`; acima de 12 mantém a oitava e soma uma quinta. É o único som que **sobe pela mesa inteira**: o turno 20 soa objetivamente mais agudo que o turno 1. Toda repetição leva `pitch_scale ±3%` e cooldown de 40 ms; `Audio` **recusa** o mesmo som no mesmo pitch duas vezes em 60 ms, e em teste isso é falha.

**Escada de sinal da compra (anti-habituação), com os tetos alinhados à §7.4:** out comum (≤ 3/5), ~55% das compras → pontinho na carta, **silêncio**; out que leva a 4/5, **≤ 26%** → pulsa 1×, rótulo pisca, `ping_out`; out que fecha cruz, **≤ 6%** → pulsa 3×, as duas linhas acendem, `ping_out` + quinta + brilho dourado. Soma **32%**, dentro do teto de 35% da §7.4. **Se o simulador medir acima de 32%, o dial permitido é elevar o limiar de 4/5 para "4/5 com categoria garantida ≥ Trinca" — nunca silenciar a cruz.**

**Barramentos:** `Master → {Musica, Efeitos, IU}`, `AudioEffectLimiter` no Master (−1 dB), pool de 16 `AudioStreamPlayer` com roubo do mais antigo. Ducking da `Musica` em −8 dB por tween de `volume_db` (ataque 120 ms, sustentação 600 ms, retorno 900 ms) em cruz, vitória e derrota — **tween, não sidechain**.
...
> **O nome mudou depois deste briefing.** A escolha original era **CRUZ**, pela razão de que nomeava a jogada. Ela caiu porque significa *guerra santa* em três línguas latinas e nada nas outras, e o título tinha que ser universal sem tradução. A jogada continua se chamando cruz. Os dezoito nomes testados e o motivo de cada queda estão em `placard/NOME.md`.
...
|---|---|---|---|
| `nucleo_testes.gd` | `testes.tscn` | avaliador (48), grade e colheita (30), cruz, Tear e teto (22), colheita final R14b (10), chefes × grau (54), economia e loja (18), determinismo (12), save e migração (10), fita (6), replay da 4.4, prévia = resultado, anti-mão-morta, âncora, pilhas | **≥ 220 asserções nomeadas, < 25 s** |
...
| **2** | `DESIGN.md` completo **antes de qualquer regra**: as 48 regras, o pipeline R40, a tabela de opcodes da §8.0, os **contratos congelados da §14.9**, todas as bordas, os cortes (Funil, Nível, Enxuta, curva `1,6^n`, redução da cruz) e as bandas de aceite | Parecer escrito do revisor adversarial em `AUDITORIA.md`: `REVISÃO F1 — <n> achados — <n> corrigidos — <n> dívidas com teste` |
| **3** | Núcleo de pontuação: `cartas`, `maos`, `avaliacao`, `outs`, `rotulos`, `receita` | 48 asserções do avaliador + `_quina_pela_rota_do_espelho` |
| **4** | Núcleo de mesa: `grade`, `mesa`, `aleatorio`, `fita`, colheita, **cruz com teto visível**, Tear, **colheita final R14b**, **diagonais R03b**, invariante R19 e R19b | Replay 4.4 dá **4.144 pontos**; `_fita_paga_antes_de_doer`, `_previa_igual_ao_resultado` e `_nunca_mao_morta` verdes; 0 travamentos em 300 seeds |
...
2. **Em 60 s entendi a regra sem ler nada** — vi uma colheita **arrancar** carta de cada perpendicular e uma cruz somar mults, com ≤ 12 palavras na tela por passo. Aos 46 s a grade abre em 5×5 e a run começa **sem clique**: o menu só aparece se eu apertar `Esc`.
...
8. **Perco e não fico sem nada** — dinheiro proporcional, uma Fiança acesa (só por perda que eu não escolhi), Novelos, um carimbo fantasma da cruz que estava na mesa e, ao repetir, **+1 posicionamento**.
...
11. **Faço uma cruz e o jogo para para olhar** — câmera lenta, duas (ou três) escadas de notas, `×4 + ×8 = ×12`, e **nenhum modal antes**.
...
15. **Jogo em pé, no celular** — em 360×800 os rótulos continuam legíveis como barras simbólicas, o toque longo abre a prévia e a cruz vibra 55 ms.
...
7. Modal no loop de turno, aviso de demolição antes de cruz, ou mais de 6 palavras num balão do loop.
...
2. **`placard/DESIGN.md`** — escrito **antes** do código e atualizado ao fim: as 48 regras, o pipeline R40, a **tabela de opcodes da §8.0**, os **contratos congelados da §14.9**, todas as regras de borda por selo e por par de selos, os cortes declarados (Funil, relíquia Nível, modificador Enxuta, curva `1,6^n`, redução 60/35 da cruz) **com o motivo**, as bandas de aceite, a prova numérica da razão da cruz (**1,63**), a semântica completa da R14b e a prova do invariante anti-travamento.
3. **`cruz/README.md`** — o jogo em 5 linhas, como rodar, como testar (`bash testar.sh` e `bash testar.sh --completo`), como exportar, o que existe (Lotes 0 e 1) e o que **não** existe (Lote 2, itens cortados pela §16.1, limitação do preset Windows, ausência de rede, `metricas.csv` local e apagável).
4. **`cruz/AUDITORIA.md`** — motor resolvido e versão real; seção `API CONFERIDA` com os 15 suspeitos da §3.3; tabela de **decisões** tomadas sem perguntar, com critério e data; tabela do **DoD** com os 30 itens, o comando executado e a saída colada; relatório de balanceamento com as métricas medidas (e as bandas não atingidas, se houver); `CORTE POR ESCOPO` para cada item cortado pela §16.1; `EXPORT — pulado`, `CAPTURAS — puladas`, `PUSH — falhou` e `BLOQUEIO EXTERNO`, quando aplicáveis; dívidas conhecidas, cada uma com o teste que a cobre.
5. **`placard/capturas/`** — as 18 capturas (17 em 1280×720 + `partida_360x800.png`), versionadas, mais um cartão de partida de exemplo. `balanceamento.csv` e `sensacao.csv` são gerados e ficam fora do git.
6. **`cruz/SENSACAO.md`** — as 5 respostas da Etapa 11b.
...
9. **Commit** em `feat/cruz`, criada a partir da branch atual — **nunca** commit direto na branch padrão. `push` quando houver remoto; falhando, `PUSH — falhou — <motivo>` em `AUDITORIA.md` e o item conta como cumprido. Mensagem descrevendo o entregue, os cortes da §16.1 e o resultado dos dois modos do `testar.sh`.
