...
# de onde veio o valor da decisao: o Joab decidiu / saiu de medicao / recomendacao aceita
ORIGENS = ("👤", "📏", "🤖")
...
def ler(jogo: Path) -> dict[str, str]:
    ...
    for apelido, relativo in DOCUMENTOS.items():
        ...
        if caminho.is_file():
            achados[apelido] = sem_comentario(
...
def sem_comentario(texto: str) -> str:
    """Tira os blocos <!-- ... --> antes de ler.

    Sem isso, um exemplo comentado dentro do modelo vira decisao de verdade —
    foi exatamente o que aconteceu com os exemplos do modelo de PLANO.md.
    """
...
def decisoes(plano: str) -> list[dict]:
    ...
    for linha in plano.splitlines():
        ...
        origem = next((m for m in ORIGENS if m in resto), None)
        achadas.append(
            {
                "id": ident,
                "bloco": bloco,
...
def conferir(jogo: Path) -> dict:
    ...
    sem_origem = [
        f"{d['id']} {d['bloco']} {d['titulo']} — 🟢 sem marca de origem: quem decidiu?"
        for d in todas
        if d["estado"] == FECHADA and d["origem"] is None
    ]
    ...
    bloqueia_codigo = pendentes(FASES_QUE_TRAVAM_CODIGO) + palpites + sem_origem
