...
## qualquer classe que use `Bus`/`Cfg` — `Progresso`, aqui — falha a compilar.
## Carregado por `res://tools/validar_dados.gd` já dentro de `_initialize()`.
...
func rodar(cena: SceneTree) -> void:
	...
	arvore.quit(0 if erros.is_empty() else 1)
...
## `escalaComOnda` num contador que reseta pede o impossível.
##
## A meta é escalada por `recorde / 10` e o progresso é medido por delta — e
## `onda`, `nivel` e os níveis de upgrade/talento caem a cada prestígio. Pedir
## um múltiplo do recorde a um contador que anda no máximo até o recorde é uma
## missão que nunca fecha. O motor já ignora a combinação; aqui o conteúdo é
## obrigado a não declará-la, senão o JSON continua dizendo uma coisa e o jogo
## fazendo outra.
func _escala_coerente() -> void:
	for par in [["diaria", Dados.missoes_diarias], ["semanal", Dados.missoes_semanais]]:
		for mi in par[1]:
			if not bool(mi.get("escalaComOnda", false)):
				continue
			var tipo := str(mi.get("meta", {}).get("tipo", ""))
