#!/usr/bin/env bash
# SessionStart: deixa a sessao pronta sem ninguem pedir.
...
FASE="$(python3 "$RAIZ/planejamento/portao-plano.py" "$RAIZ" 2>/dev/null \
        | grep -o '"fase_alcancada": [0-9]*' | grep -o '[0-9]*' || true)"
echo "KINGSPIRE — plano na fase ${FASE:-?}. Prova: ./testar.sh"
