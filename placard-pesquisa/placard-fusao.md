...
`tear_ganho_simples/cruz` (c1), `mat_F/mat_teto/gulosa_ve_ritmo` (c2) e `n_sementes/vies/posic_ov`
...
|---|---|---|
| c1 (so a cruz levanta o Tear) | REPROVADA | gulosa 0,0000 em **todas** as 8 variantes; planejadora cai 1,4511 -> 1,3144…1,4351; Tear mediano 7 -> 5 (V1) e -> 1 (V3/V4) |
...
dela, e remedi a sensibilidade (secao 6): a leitura muda o PRECO da cruz, nao a frequencia.
...
- **Linha que nasce cheia na semeadura colhe no turno 0**, antes do jogador, mesmo com a janela
  ...
  `cruzes_por_mesa`; vai para `eventos_fantasma_por_mesa`. Medido: 0,002/mesa com S=6.
...
gulosa **0,000** cruz / 100% de mesas zeradas / rec **64,8%** / seca **2/4** / Tear **7** /
...
| J1+c1  (so cruz levanta Tear) | 4.39 | **0.831** | 0.820 | 0.857 | 1.636 | 0.805 | 16.9% | 0.708 | 75.8% | 2/3 | 7 | 32600 | 10.06 | 0.792 | 35.7 | **68.4** | 3.52 | 80.1% | **APROVADA** |
...
cruz na gulosa E m5 menor ou igual. As outras tres regras, somadas a ela, sao inertes ou negativas.

**(a) c1 e um dial de zero bit tambem DEPOIS que a cruz existe.** Esta era a hipotese mais forte
de sinergia e ela **falhou de forma medida**: c1 foi reprovado sozinho porque a gulosa nunca fazia
cruz, entao "so a cruz levanta o Tear" equivalia a "nada levanta o Tear". Com a janela ligada
a gulosa faz 0,83 cruz por mesa — o gatilho de c1 passa a existir de verdade. Resultado:
**J1 0,831 vs J1+c1 0,831**, m5 68,2 vs 68,4, distancia 0,805 vs 0,805, seg4/5 3,52 vs 3,52. Igual
ate a terceira casa. A explicacao da fase D da b1 continua valendo com a cruz acontecendo: o Tear
...
56.320, com K inflado para 7,79 e a mesma 0,831 cruz.

**(b) c2 (maturacao) e ANTI-cruz quando somada a c4, e o mecanismo e claro.** F=0,75 leva a gulosa
...
pelo mesmo turno. `seg 4/5` sobe (3,52 -> 3,74) e `%adiadas` sobe (80,2 -> 85,9), mas as cruzes
CAEM: o jogador passa a ser pago por adiar coisas que nao viram cruz. E o custo em dificuldade e
brutal: K de 4,40 para **8,83** (+101% sobre o ja inflado J1, +307% sobre a base).

**(c) c3 (semear) nao muda a cruz e gasta a guarda de profundidade.** S=6 conservado: gulosa
...
O gargalo de toda combinacao e o **m5**, nao a cruz. Ordenando as celulas por m5:
...
Corolario para o design: nao existe "empilhar melhorias de cruz". Escolhe-se **uma**.

## 6. Sensibilidade (celulas extras, sobre a regra vencedora)

| leitura | gulosa | profunda | planejadora | m5 | %cruzes com o teto R15 mordendo |
|---|---|---|---|---|---|
...
de cruz (0,826-0,831) e o mesmo m5 (68,1-68,3). Ela muda o preco. Mas o aviso do teto do mult
continua valendo com forca: sob PRODUTO o teto morderia **57,5%** das cruzes — se o painel fixar
...
O que isso entrega (1.000 mesas): gulosa **0,831** cruz/mesa com **83,1% das mesas** tendo pelo
...
2. **cruz/evento 0,710** — a cruz deixa de ser excecao e vira o modo normal de colher, e
...
3. **Planejar deixa de pagar**: a razao pontos/meta da planejadora cai de 1,126 (base) para 0,665.
   A janela entrega a cruz tao barata que o planejamento vira desperdicio. Nenhuma das outras
...
**gratuita em cruz** (0,831, m5 68,5): ela nao muda o jogo, so devolve Tear mediano 8 e um teto
de evento mais alto para a fantasia do chefe 6. Nao a recomendo — e um dial de zero bit medido
duas vezes, agora tambem com a cruz acontecendo.
...
- O criterio "intencional x acidental" do briefing continua quebrado (marca ~100% de acidente em
  toda politica, inclusive na planejadora, porque o gatilho da cruz e quase sempre tambem a
...
- Reportar cruz por mesa sozinha continua enganoso. Com a janela, `cruzes_por_evento` = 0,710:
  o numero por mesa sobe em parte porque ha **menos eventos**, nao so mais cruzes.
